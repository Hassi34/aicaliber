...
def get_data_and_features(df, auto):
    ...
    if auto == False:
        usr_rsp = input('   == Please type "yes" if you agree with the above selection otherwise, type "no" : ')
        if usr_rsp.title() == "Yes" :
            print(f'    == Continuing with default selection...')
        elif usr_rsp.title() == "No":
            num_features = input("Enter a list of numerical features, hit enter if there are none : ")
            cat_features = input("Enter a list of categorical features, hit enter if there are none : ")
            print(cat_features)
            print(type(cat_features))
            num_features = num_features[1:-1].split(',')
            cat_features = cat_features[1:-1].split(',')
            if len(cat_features[0]) > 0:
                cat_features = [(element.replace("'", "").replace('"', '')).strip() for element in cat_features]
            else :
                cat_features = []
            if len(num_features[0]) > 0:
                num_features = [(element.replace("'", "").replace('"', '')).strip() for element in num_features]
            else:
                num_features = []
            ...
            df = pd.concat([cat_data, num_data], axis = 1)
            #elif cat_data is None:
             #   df = num_data 
            #lif num_data is None:
             #   df = cat_data
        else:
