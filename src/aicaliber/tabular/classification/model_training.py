...
class TrainingFlow:
    ...
    def compare_base_classifiers(self):
        ...
        for i in tqdm(self.models):
            ...
            metrics_dict['acc_train'].append(round(accuracy_score(self.y_train, y_predicted_train), 4))
            metrics_dict['acc_val'].append(round(accuracy_score(self.y_val,y_predicted),4))

            metrics_dict['f1_train'].append(round(f1_score(self.y_train, y_predicted_train, average = 'weighted'),4))
            metrics_dict['f1_val'].append(round(f1_score(self.y_val,y_predicted, average = 'weighted'),4))
            
            metrics_dict['recall_train'].append(round(recall_score(self.y_train, y_predicted_train, average = 'weighted'),4))
            metrics_dict['recall_val'].append(round(recall_score(self.y_val,y_predicted, average = 'weighted'),4))

            metrics_dict['precision_train'].append(round(precision_score(self.y_train, y_predicted_train, average = 'weighted'),4))
            metrics_dict['precision_val'].append(round(precision_score(self.y_train, y_predicted_train, average = 'weighted'),4))

        self.model_training_results = pd.DataFrame(metrics_dict, index= self.models.keys()).sort_values(by=self.sort_by, ascending=False)
    ...
    def base_model_report(self):
        ...
        print("*****" * 13)
        print(classification_report(y_predicted, self.y_val, target_names = self.target_names))
    ...
    def hyper_param_tuninig(self):
        ...
        param_grid, scoring = get_heper_param_grid(self.best_model, self.metrics)
    ...
    def get_best_model(self):
        if self.auto:
            ...
            print('\n')
            if usr_rsp.title() == 'No':
                usr_rsp = input("   == Please enter a number(integer) for top N Models to run hyperparameter tuning for or pass an exact model name(lookup in the above result table) to tune a single specific model :")
                if len(usr_rsp) == 1:
                    top_n = int(usr_rsp)
